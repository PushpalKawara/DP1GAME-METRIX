...
if file1 and file2:
    ...
    fig, ax = plt.subplots()
    ax.plot(df1['LEVEL_CLEAN'], df1['Retention %'], marker='o', linestyle='-')
    ax.set_xlabel("Level")
    ax.set_ylabel("Retention %")
    ax.set_title("User Retention by Level")
    ax.grid(True)
    st.pyplot(fig)
    ...
    with tab1:
        ...
        if st.button("Update Summary Table"):
            df_summary["Day 1 Retention"] = [day1_retention]
            df_summary["Day 3 Retention"] = [day3_retention]
            df_summary["Session Length"] = [session_length + " s"]
            df_summary["Playtime length"] = [playtime_length + " s"]
